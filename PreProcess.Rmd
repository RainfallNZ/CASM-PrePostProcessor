...
```
I need to analyse each management sub zone to determine how much area each landuse-LUC combination takes up
```{r}
#Create a new class that combines the submanagement zone (ZONE_CODE), the reclassified landsue (RegSciLand) and the primary Land Use Capability class (LUC_CLASS)
SubZoneLanduseLUCSpatial@data$ZoneLanduseLUC <- paste0(SubZoneLanduseLUCSpatial$Zone_Code,"-",SubZoneLanduseLUCSpatial$RegiScLand,"-",SubZoneLanduseLUCSpatial$LUC_CLASS)

#Dissolve the polygons with the same zone-landuse-LUC combination
SimplifiedSubZoneLandusLUCSpatial <- unionSpatialPolygons(SubZoneLanduseLUCSpatial, SubZoneLanduseLUCSpatial$ZoneLanduseLUC)

#Find the area of each Landuse-LUC combination for each sub-zone
SimplifiedSubZoneLandusLUCSpatial$area <- area(SimplifiedSubZoneLandusLUCSpatial)

test <- st_as_sf(SubZoneLanduseLUCSpatial)

df_new <- test %>%
  group_by(ZoneLanduseLUC) %>%
  summarise_all(first) 
#df_new
...
```
Then create a point source table ready for CASM, and a measurementsite table ready for CASM
```{r}
...
addWorksheet(Out, "Water Quality Sattions")

writeData(Out, sheet = "Water Quality Sattions", x = MeasurementSiteTable[,-1])
