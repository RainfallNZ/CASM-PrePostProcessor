...
#'
#'This function generates a vector of the reach ID numbers (NZREACH attribute) downstream of a reach. It uses the REC V2.4 network. See \href{https://niwa.co.nz/freshwater-and-estuaries/management-tools/river-environment-classification-0}{NIWA REC V2} for details about REC V2.
#'
#'@param RECNetwork An REC V2 network (either dataframe of spatial dataframe), with at least NZREACH, NZTNODE and NZFNODE attributes
#'@param SourceReach The reach number (i.e. NZREACH attribute) of the reach for which the downstream network is required
#'@author Tim Kerr, \email{Tim.Kerr@@Rainfall.NZ}
#'@return A vector of reach numbers (NZREACH attributes)
#'@keywords REC River Environment Classification
#'@export
DownstreamReachFinder <- function(RECNetwork=RECReachNetwork,SourceReach=7039931){ #The reach ID is for the Manawatu at Teachers College water quality reporting site
  ...
  SourceSiteReachIndex<-which(RECNetwork$NZREACH==SourceReach)
  
  #Identify the reach immediately downstream of the source reach
  downstream_index<-which(RECNetwork$NZFNODE==RECNetwork$NZTNODE[SourceSiteReachIndex])
  
  #downstream_reach<-RECNetwork$nzsegment[downstream_index]
  
  #Sequentially work downstream
  while (length(downstream_index)>0) {
     DownstreamReaches <- c(DownstreamReaches,RECNetwork$NZREACH[downstream_index])
     downstream_index<-which(RECNetwork$NZFNODE==RECNetwork$NZTNODE[downstream_index])
...
NetworkLabeler <- function(RECNetwork=MyREC){
  ...
  OutletReachIndices <- which(!RECNetwork$NZTNODE %in% RECNetwork$NZFNODE)
  ...
  ReachLabels <- lapply(OutletReachIndices, function(OutletReachIndex) {   #OutletReachIndex <- 39
    ...
    RowNumber          <- 1
    CurrentLabel       <- 1
    Labels             <- data.frame(NZREACH=NA,Label=NA)
    Labels[RowNumber,] <- c(RECNetwork$NZREACH[OutletReachIndex],1)
    
    CurrentReachIndex  <- OutletReachIndex
    upstream_indices   <- which(RECNetwork$NZTNODE==RECNetwork$NZFNODE[CurrentReachIndex])
    LeftToDoIndices    <- upstream_indices
    
    while (length(LeftToDoIndices) > 0) {
      #Check if there are some upstream indices
      BiggestBranchIndex <- which.max(RECNetwork$CATCHAREA[upstream_indices])
      ...
      Labels[RowNumber,] <- c(RECNetwork$NZREACH[CurrentReachIndex],CurrentLabel)
      upstream_indices   <- which(RECNetwork$NZTNODE==RECNetwork$NZFNODE[CurrentReachIndex])
      if (length(upstream_indices) == 0) {
        CurrentLabel = CurrentLabel + 1
        upstream_indices <- LeftToDoIndices[which.max(RECNetwork$CATCHAREA[LeftToDoIndices])]
      } else {
...
TributaryConnectionCreator <- function(RECNetwork = CompleteSpatialNetwork, TributaryLabelList = TribLabelList){
  ...
  CatchmentTribLinkages <- lapply(seq_along(TributaryLabelList), function(CatchmentIndex) { 
    ...
    #for each tributary find the minimum DISTSEA attribute from the REC data, the REC reach immediately downstream of the tributary, and the tributary label of the REC reach that is immediately downstream. This builds a matrix of 4 numbers for each tributary, giving the minimum DISTSEA, the lowest NZREACH, the highest NZREACH below the tributary, and the label of the tributary below (i.e. the confluence name)
    ConfluenceTotalDistances <- sapply(UniqueTribs, function(TribLabel) {
      
      #Get the REC data for the current tributary
      ReachData <- RECNetwork[RECNetwork$NZREACH %in% CatchmentTribLabels$NZREACH[CatchmentTribLabels$Label == TribLabel],]
      
      #Find the smallest DISTSEA attribute for the tributary
      TribConnectionTotalDistance <- min(ReachData$DISTSEA)
      
      #Find which reach is the lowest in the tributary
      LowestReach <- ReachData$NZREACH[which.min(ReachData$DISTSEA)]
      
      #Find the reach immediately below the lowest reach
      ReachBelow <- RECNetwork$NZREACH[which(RECNetwork$NZFNODE == RECNetwork$NZTNODE[RECNetwork$NZREACH == LowestReach])]
      
      #Special case if it is the lowest tributary, as it is effectively the mainstem
      if(length(ReachBelow)==0) {
        ReachBelow <- NA
        TribBelow <- 1 }
      else {TribBelow <- CatchmentTribLabels$Label[CatchmentTribLabels$NZREACH == ReachBelow]}
      
      return(c(TribConnectionTotalDistance,LowestReach,ReachBelow,TribBelow))
    })
    
    #CASM needs the distance of a confluence above the previous confluence. So the DISTSEA of the bottom of the parent tributary needs to be subtracted from the DISTSEA of the current tributary. This is achieved by using the tributary values just created for each catchment.
...
MeasurementSites <- HZLoads[!duplicated(HZLoads$NZReach),]
...
#Load the point sources of nitrogen, previously prepared by Caroline Fraser. Unfortunately they don't have NZREACH data.
PointSourceSites <- read.csv(file.path(DataDirectory,"PointSourcesSummaryPC2_YE2012_04.csv"))
#Add a NZREACH attribute and look them up from the REC V2 data.
PointSourceSites$NZREACH <- RECReachNetwork$NZREACH[RECReachNetwork$REACH_ID%in%PointSourceSites$nzsegment]
#Create a spatial points object set to NZTM
...
SubZonePolyons <- readOGR(dsn =file.path(GISDataDirectory,REC_rivers),
...
```{r}
AllPoints <- c(MeasurementSites$NZReach, PointSourceSites$)
LoadNetwork <- lapply(MeasurementSites$NZReach, function(x) {DownstreamReachFinder(RECNetwork = RECReachNetwork, SourceReach = x)} )
