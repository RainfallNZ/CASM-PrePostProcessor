...
LeachRateRasterCreator <- function(ClimateData=file.path(GISDataDirectory,ClimateShapeFileName),
                                   ...
                                   LanduseToLanduseLUT = file.path(DataDirectory,LanduseToLanduseLUTFile)){
                                     ...
                                     RasterBase <- raster(resolution = 100, ext = extent(ClimateSpatial), crs = proj4string(ClimateSpatial) )
                                     ...
                                     # write.table(UniquePredictorCombinations, file.path(DataDirectory,"UniquePredictorCombinations.csv"),row.names = FALSE, sep=",")
                                     # library(dplyr)
...
I need to rasterize the zone-landuse-LUC data, and associate the leach rate with each zone-landuse-LUC combination.
I can then find the average leach rate and area for each combination.
```{r}
DiffuseLoadTableCreator <- function(ZoneLanduseLUCPolygons=SubZoneLanduseLUCSpatial,
                                   LeachRates=LeachRateRaster){
                                     ...
                                     ClassSummaries <- ddply(RasterData,.(CombinedClassNameLevel), summarise, LeachRate =round(mean(LeachRates),1), count =  length(LeachRates), Hectares = count * CellAreaHectares)
...
MPILeachRateLUTFile <- "MPI_Baseline_N_losses_v3.csv"
...
##Get the average leach rate for each polygon THIS TAKES ABOUT 5 MINUTES for 1km x 1km and 46 mins for 500 m x 500 m
tic()
...
#Add the leach rates
#First option uses the leach values that were obtained by sampling the leach rate raster at each polygon
#DiffuseInputsSiteExtendedTable$"LeachRates" <-round( #leachvalues@data$layer[match(DiffuseInputsSiteExtendedTable$CombinedClassName,leachvalues@data$CombinedClassName)],1)

#Second option uses leach rates that need to be obtained by rasterising the zone-landuse-LUC combinations
DiffuseLeachRateData <- DiffuseLoadTableCreator(ZoneLanduseLUCPolygons = SubZoneLanduseLUCSpatial,LeachRates = LeachRateRaster)
...
DiffuseInputsSiteExtendedTable$`Land Area (ha)` <- DiffuseLeachRateData$Hectares[match(DiffuseInputsSiteExtendedTable$CombinedClassName,DiffuseLeachRateData$CombinedClassName)]
...
browser()
#rename the columns to match the CASM conventions
names(DiffuseInputsSiteExtendedTable) <- c("Landuse","LUC","Zone_Code","Land Area (ha)","Node Name","Original location (km)", "Receiving Stream","Export Coeff (kg/ha/yr)","Discharge Location (km)")
...
```
 By way of a check, it will be helpful to compare the total sub-management zone loads from the griddeed leachrate data with the total sub-management zone loads from the polgonised zone/landuse/LUC data.
 Start with the raster data, and get the average leachrate for each zone and multiply by the area.
 Note that  I need to use the subzones from the SubZone
 
 Then, for the polygonised data, get the load for each polygon, and sum all the polygons in each zone
```{r}
