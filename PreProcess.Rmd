...
LeachRateRaster <- raster(file.path(GISDataDirectory,LeachRateRasterFileName))

#**********************************************************
##If the leach rate raster is missing or needs to be re-created then use the following:
##LeachRateRaster <- LeachRateRasterCreator()
##And save it for nexttime
#writeRaster(LeachRateRaster,file.path(GISDataDirectory,LeachRateRasterFileName),overwrite=TRUE)
#******************************************************************
```
...
SubZoneLanduseLUCSpatial$hectares <- raster::area(SubZoneLanduseLUCSpatial) / 10000
...
#Get the average leach rate for each polygon
leachvalues <- raster::extract(LeachRateRaster, SubZoneLanduseLUCSpatial, fun = mean, na.rm=TRUE, sp=TRUE)
```
...
MeasurementSiteTable$ExpCoef <- round(MeasurementSites$ExpCoef[match(MeasurementSiteTable$"Site Name or No", MeasurementSites$sID)],1)
MeasurementSiteTable$ExpCoef_Lci <- round(MeasurementSites$ExpCoef_Lci_[match(MeasurementSiteTable$"Site Name or No", MeasurementSites$sID)],1)
MeasurementSiteTable$ExpCoef_Uci <- round(MeasurementSites$ExpCoef_Uci_[match(MeasurementSiteTable$"Site Name or No", MeasurementSites$sID)],1)
```
