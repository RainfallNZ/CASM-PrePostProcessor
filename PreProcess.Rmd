...
#Work through each management zone to find the one with the least distance to the sea (the LENGTHD attribute)
ManagementSubZoneOutletReaches <- lapply(seq_along(SubZonePolygons$Zone_Code), function(SingleSubZoneIndex){
  ...
  OutletReach <- CurrentSubZoneReaches$nzsgmnt[which.min(CurrentSubZoneReaches$LENGTHD)]
...
```{r}
xlsx::write.xlsx(TributaryConnectionTable,file.path(DataDirectory,"CASM-Inputs.xlsx"),col.names = TRUE, sheetName="River Network")
```
