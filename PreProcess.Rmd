...
I also need a function which combines PAW, irrigated land, irrigable land, climate and landuse and 
...
SubZoneLanduseLUCShapeFileName <- "ZoneLanduseLUC"

IrrigableLandShapeFileName <- "IrrigableLand"

IrrigatedLandShapeFileName <- "mfe-irrigated-land-area-2017-SHP/irrigated-land-area-2017-Manawatu"

PAWShapeFileName <- "niFSL_fixedgeom"

ClimateShapeFileName <- "ClimateRegions"

MPILeachRateLUTFile <- "MPI_Baseline_N_losses_v1.csv"
...
SubZoneLanduseLUCSpatial <- readOGR(dsn =file.path(GISDataDirectory,SubZoneLanduseLUCShapeFileName),
                           layer = SubZoneLanduseLUCShapeFileName)
SubZoneLanduseLUCSpatial <- spTransform(SubZoneLanduseLUCSpatial,CRS("+init=epsg:2193") )



#Load climate spatial data
ClimateSpatial <- readOGR(dsn =file.path(GISDataDirectory,ClimateShapeFileName),
                           layer = "ClimateRegions-Horizon")
#Convert to raster
RasterBase <- raster(resolution = 1000, ext = extent(ClimateSpatial), crs = proj4string(ClimateSpatial) )
ClimateRaster <- rasterize(ClimateSpatial,RasterBase)

#Load Irrigable land spatial data
IrrigableLandSpatial <- readOGR(dsn =file.path(GISDataDirectory,IrrigableLandShapeFileName),
                           layer = "Irrigable-Horizons")
IrrigableRaster <- rasterize(IrrigableLandSpatial, RasterBase, rep(1,length(IrrigatedLandSpatial)))

#Load irrigated land spatial data
IrrigatedLandSpatial <- readOGR(dsn =file.path(GISDataDirectory,"mfe-irrigated-land-area-2017-SHP"),
                           layer = "irrigated-land-area-2017-Manawatu")
IrrigatedRaster <- rasterize(IrrigatedLandSpatial, RasterBase, rep(1,length(IrrigatedLandSpatial)))

#Load Plant Available Water (PAW) spatial data
PAWSpatial <- readOGR(dsn =file.path(GISDataDirectory,PAWShapeFileName),
                           layer = "FSL_Horizons", stringsAsFactors = FALSE)
#Convert PAW values to numbers (I don't know why they are characters)
PAWSpatial$PAW_Cat2 <- as.numeric(PAWSpatial$PAW_Cat2)
PAWRaster <- rasterize(PAWSpatial,RasterBase)

#Load MPI lookup table of total nitrogen leaching rates based on climate, land use, PAW, irrigation, irrigable land
MPILeachRateLookUpTable<- read.table(file.path(DataDirectory,MPILeachRateLUTFile),sep=",")

#Load MPI lookup table of the names fo r the classes used to estimate the leach rate
#This is needed to convert the various input spatial data (landuse, climate etc.) into the number-based classification used by the MPI leach rate look up table
MPILeachClassNameLookUpTable<- read.table(file.path(DataDirectory,MPIClassNameLUTFile),sep=",")
```
...
SubZoneLanduseLUCSpatial$hectares <- area(SubZoneLanduseLUCSpatial) / 10000
...
#add mean flow data to the measurement site data
MeasurementSitesSpatial@data$QMean <- 
