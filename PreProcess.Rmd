...
```{r}
NetworkLabeler <- function(MyREC=MyREC){    #MyREC <- CompleteSpatialNetwork

  
  #Find the row indices of all the outlet reach's by finding which "to" nodes  don't have a corresponding "from" node
  OutletReachIndices <- which(!MyREC$NZTNODE %in% MyREC$NZFNODE)
  ...
  ReachLabels <- lapply(OutletReachIndices, function(OutletReachIndex) {   #OutletReachIndex <- 39
    ...
    Labels[RowNumber,] <- c(MyREC$NZREACH[OutletReachIndex],1)
    
    CurrentReachIndex  <- OutletReachIndex
    upstream_indices   <- which(MyREC$NZTNODE==MyREC$NZFNODE[CurrentReachIndex])
    LeftToDoIndices    <- upstream_indices
    
    while (length(LeftToDoIndices) > 0) {
      #Check if there are some upstream indices
      BiggestBranchIndex <- which.max(MyREC$CATCHAREA[upstream_indices])
      ...
      Labels[RowNumber,] <- c(MyREC$NZREACH[CurrentReachIndex],CurrentLabel)
      upstream_indices   <- which(MyREC$NZTNODE==MyREC$NZFNODE[CurrentReachIndex])
      if (length(upstream_indices) == 0) {
        CurrentLabel = CurrentLabel + 1
        upstream_indices <- LeftToDoIndices[which.max(MyREC$CATCHAREA[LeftToDoIndices])]
      } else {
